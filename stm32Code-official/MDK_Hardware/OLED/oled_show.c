...
static void _Manual_Sub(const char* name_gbk, uint8_t* relayState)
{
	...
	/* Show title: "手动控制" or "喂食中" */
	if (Flag.feeding) {
		/* Show feeding status */
		ZoneBitCode_OLED_Show_ChineseF6X12(2, 0,  (uint8_t*)"\xce\xb9", 0);  /* 喂 */
		ZoneBitCode_OLED_Show_ChineseF6X12(2, 12, (uint8_t*)"\xca\xb3", 0);  /* 食 */
		OLED_Show_OneCharF6X12(2, 24, ':');
