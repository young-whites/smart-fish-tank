...
static void draw_page_indicator(void)
{
    ...
    oled_draw_string_8x16(96, 6, buf);
}
...
static void draw_main_p1_data(void)
{
    ...
    oled_draw_string_8x16(0, 0, (g_sensor.run_mode == MODE_AUTO) ? "Mode:  Auto" : "Mode:  Manu");
    ...
    oled_draw_string_8x16(0, 2, "WiFi:  W-");
...
static void draw_threshold_p0_data(void)
{
    ...
    oled_draw_string_8x16(0, 0, "-- Threshold Set --");
...
static void draw_manual_data(void)
{
    ...
    oled_draw_string_8x16(0, 0, "-- Manual Control --");
    ...
    for (i = 0; i < MANUAL_ITEMS; i++) {
        ...
        rt_snprintf(buf, sizeof(buf), "%c %s: %3s %s",
                    ...
                    (i == edit_cursor) ? "<<" : "");
        oled_draw_string_8x16(0, row, buf);
    }
