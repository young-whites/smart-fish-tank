...
static void _KEYE_LongPress(void)  // 闀挎寜2s瑙﹀彂鍠傞??
{
	if (Flag.feeding == 0) {
		Flag.feeding = 1;
		Record.feedCountdown = 30;  // 榛樿??30绉掑?掕?℃椂
	}
}
...
/*****************************************************************************
* 鍚嶇О:		鎸夐敭浜嬩欢澶勭悊涓诲嚱鏁?
* 鍔熻兘:		浠庢寜閿?缂撳啿鍖鸿?诲彇閿?鍊硷紝鍒嗗彂鍒板?瑰簲鎸夐敭澶勭悊鍑芥暟
*****************************************************************************/
extern void (*current_operation_index)(void);
void KEY_Scan(void)
{
	uint8_t	key, event;
	for (key = KEY_Read(); key; key = KEY_Read())  // 閫愪釜璇诲彇缂撳啿鍖洪敭鍊肩洿鍒扮紦鍐插尯涓虹┖
	{
		event = key & 0xf0;		// 浜嬩欢绫诲瀷
		key   = key & 0x0f;		// 閿?鍊?
		switch (key)
		{
			case KEY_A:  // KEY1 - PA8 - 椤甸潰鍒囨崲锛堝彧鏈夋寜涓嬩簨浠讹級
			{
				switch (event)
				{
					case KEY_Evt_Press:		_KEYA_Press();	break;	// 鎸変笅
				}
			}break;

			case KEY_B:  // KEY2 - PB3 - 闃堝?煎弬鏁伴?夋嫨锛堝彧鏈夋寜涓嬩簨浠讹級
			{
				switch (event)
				{
					case KEY_Evt_Press:		_KEYB_Press();	break;	// 鎸変笅
				}
			}break;

			case KEY_C:  // KEY3 - PB4 - 鍔?/妯″紡鍒囨崲锛堢煭鎸?=鍔狅紝闀挎寜=妯″紡鍒囨崲锛?
			{
				switch (event)
				{
					case KEY_Evt_Press:		_KEYC_Press();	break;	// 鐭?鎸?=鍔?
					case KEY_Evt_Long2S:	_KEYC_LongPress();	break;	// 闀挎寜=妯″紡鍒囨崲
				}
			}break;

			case KEY_D:  // KEY4 - PB8 - 鍑忥紙鍙?鏈夋寜涓嬩簨浠讹級
			{
				switch (event)
				{
					case KEY_Evt_Press:		_KEYD_Press();	break;	// 鎸変笅=鍑?
				}
			}break;

			case KEY_E:  // KEY5 - PB1 - 鍠傞?燂紙闀挎寜2绉掕Е鍙戯級
			{
				switch (event)
				{
					case KEY_Evt_Long2S:	_KEYE_LongPress();	break;	// 闀挎寜瑙﹀彂鍠傞??
				}
			}break;
		}
	}
